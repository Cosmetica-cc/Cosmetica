...
public class Models {
	...
	private static Map<ResourceLocation, BlockModel> LOADED_MODELS = new HashMap<>();
	...
	public static void loadTestResource() {
		if (!BAKED_MODELS.containsKey(TEST_LOCATION)) {
			Cosmetics.runOffthread(() -> {
				getBlockModel(TEST_LOCATION);
			});
		}
	}

	public static void loadBakedModel(BlockModel model, TextureAtlasSprite sprite) {
		ResourceLocation location = new ResourceLocation(model.name);
		BakedModel result = model.bake(thePieShopDownTheRoad, l -> l.texture().getNamespace().equals("minecraft") ? Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(l.texture()) : sprite, BlockModelRotation.X0_Y0, location);
	...
	private static BlockModel getBlockModel(ResourceLocation location) {
		return LOADED_MODELS.computeIfAbsent(location, l -> {
			try (InputStream is = new FileInputStream(new File(FabricLoader.getInstance().getGameDirectory(), "test.json"))) { // TODO not use test.json
				BlockModel result = BlockModel.fromStream(new InputStreamReader(is, StandardCharsets.UTF_8));
				result.name = l.toString();
				return result;
			} catch (IOException e) {
				return null;
			}
